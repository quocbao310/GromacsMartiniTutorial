...
;energygrp-excl=Protein Protein Membrane Membrane
...
# Run
M="gmx_mpi mdrun -deffnm $BASE -v -ntomp 4"
echo $M; $M 
...
__MDP__
  ...
  # Perform run
  M="gmx_mpi mdrun -deffnm $BASE -v -ntomp $NP"
  echo $M; $M 
...
do
  ...
  echo $G; $G || exit
  M="gmx_mpi mdrun -deffnm $tag -v -ntomp $NP"
  echo $M; $M 
  GRO=$tag.gro
